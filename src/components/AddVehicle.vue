        ...
        <v-card-text class="dialog-content">
          ...
          <div class="form-group">
            ...
            <textarea
              ...
              placeholder="Describe your vehicle, organization, project, or method (optional)"
              class="form-textarea"
              rows="4"
            ></textarea>
          </div>
...
export default {
  name: "AddVehicle",
  emits: ["vehicle-added", "vehicle-updated"],
  props: {
    editMode: {
      type: Boolean,
      default: false,
  ...
  computed: {
    ...
    canSave() {
      return this.vehicleData.title.trim();
    },
  },
  methods: {
    openDialog() {
    ...
    openEditDialog(vehicle) {
      this.vehicleData = {
        ...
        relatedLinks: [...(vehicle.relatedLinks || [])],
      };
      this.dialogOpen = true;
    },
    closeDialog() {
    ...
    addLink() {
      ...
      if (trimmedLink && !this.vehicleData.relatedLinks.includes(trimmedLink)) {
        this.vehicleData.relatedLinks.push(trimmedLink);
        this.newLink = "";
      }
    },
    removeLink(link) {
      const index = this.vehicleData.relatedLinks.indexOf(link);
      if (index > -1) {
        this.vehicleData.relatedLinks.splice(index, 1);
      }
    },
    getChipColor(link) {
    ...
    saveVehicle() {
      if (this.canSave) {
        const vehicleData = {
          title: this.vehicleData.title.trim(),
        ...
        if (this.editMode) {
          ...
          this.$emit("vehicle-added", vehicleData);
        }
        this.closeDialog();
      }
    },
